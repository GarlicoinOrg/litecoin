...
def main():
    ...
    tmpdir = "%s/honeycomb_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    if not (enable_wallet and enable_utils and enable_bitcoind):
        print("No functional tests to run. Wallet, utils, and honeycombd must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[]):
    ...
    try:
        if subprocess.check_output(["pidof", "honeycombd"]) is not None:
            print("%sWARNING!%s There is already a honeycombd process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    if "GARLICOIND" not in os.environ:
        os.environ["GARLICOIND"] = build_dir + '/src/honeycombd' + exeext
