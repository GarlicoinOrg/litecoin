...
*Setup instructions for a Gitian build of Honeycomb Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Honeycomb
...
- [Building Honeycomb Core](#building-honeycomb-core)
...
Clone the git repositories for honeycomb and Gitian.
...
in the honeycomb repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Honeycomb Core
----------------

To build Honeycomb Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the honeycomb repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/honeycomb/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/honeycomb-project/honeycomb
...
For example:
```bash
URL=https://github.com/thrasher-/honeycomb.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit honeycomb=${COMMIT} --url honeycomb=${URL} ../honeycomb/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit honeycomb=${COMMIT} --url honeycomb=${URL} ../honeycomb/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit honeycomb=${COMMIT} --url honeycomb=${URL} ../honeycomb/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the honeycomb git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../honeycomb/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/honeycomb
SIGPATH=/some/root/path/honeycomb-detached-sigs

./bin/gbuild --url honeycomb=${BTCPATH},signature=${SIGPATH} ../honeycomb/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/honeycomb-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/honeycomb-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/honeycomb-osx-build.assert
```
...
[honeycomb-project/gitian.sigs.ltc](https://github.com/honeycomb-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
