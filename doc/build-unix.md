UNIX BUILD NOTES
====================
Some notes on how to build Honeycomb Core in Unix.

(for OpenBSD specific instructions, see [build-openbsd.md](build-openbsd.md))

Note
---------------------
Always use absolute paths to configure and compile honeycomb and the dependencies,
...
This will build honeycomb-qt as well if the dependencies are met.
...
memory available when compiling Honeycomb Core. On systems with less, gcc can be
...
See the section "Disable-wallet mode" to build Honeycomb Core without wallet.
...
If you want to build Honeycomb-Qt, make sure that the required packages for Qt development
...
Once these are installed, they will be found by configure and a honeycomb-qt executable will be
built by default.
...
Notes
-----
The release is built with GCC and then "strip honeycombd" to strip the debug
...
# Pick some path to install BDB to, here we create a directory within the honeycomb directory
...
# Configure Honeycomb Core to use our own-built instance of BDB
cd $GARLICOIN_ROOT
./autogen.sh
...
* Non-executable Stack
    ...
    vulnerable buffers are found. By default, honeycomb should be built with a non-executable stack
...
Disable-wallet mode
--------------------
When the intention is to run only a P2P node without a wallet, honeycomb may be compiled in
...
As mentioned above, when maintaining portability of the wallet between the standard Honeycomb Core distributions and independently built
...
with 4.8-built Honeycomb Core is needed follow the steps under "Berkeley DB" above.
